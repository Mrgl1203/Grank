 ...
 */
package com.gulei.common.utils.progressmanager;
...
import com.gulei.common.utils.progressmanager.body.ProgressRequestBody;
import com.gulei.common.utils.progressmanager.body.ProgressResponseBody;
...
public final class ProgressManager {
    ...
    public void removeRequestListener(String url) {
        checkNotNull(url, "url cannot be null");
        synchronized (ProgressManager.class) {
            if (mRequestListeners.containsKey(url)) {
    ...
    //无法在初始化时注册，避免添加多次相同注册，添加手动移除方法
    public void removeResponseListener(String url) {
        checkNotNull(url, "url cannot be null");
        if (mResponseListeners.containsKey(url)) {
            synchronized (ProgressManager.class) {
